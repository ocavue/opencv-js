...
class Builder:
    ...
    def get_cmake_cmd(self):
        ...
        if not self.options.cmake_option or all(
            ["-DCMAKE_TOOLCHAIN_FILE" not in opt for opt in self.options.cmake_option]
        ):
    ...
    def get_build_flags(self):
        flags = "-s ENVIRONMENT=worker -s EXPORT_ES6=1 -s MODULARIZE=1 "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--disable_single_file",
        action="store_true",
        help="Do not merge JavaScript and WebAssembly into one single file",
    )
    parser.add_argument(
        "--threads",
        action="store_true",
    ...
    parser.add_argument(
        "--simd", action="store_true", help="Build OpenCV.js with SIMD optimization"
    )
    parser.add_argument("--build_test", action="store_true", help="Build tests")
    parser.add_argument(
        "--build_perf", action="store_true", help="Build performance tests"
    )
    parser.add_argument("--build_doc", action="store_true", help="Build tutorials")
    parser.add_argument(
        "--build_loader", action="store_true", help="Build OpenCV.js loader"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--build_wasm_intrin_test", action="store_true", help="Build WASM intrin tests"
    )
    ...
    transformed_args = [
        "--cmake_option={}".format(arg) if arg[:2] == "-D" else arg
    ...
    if args.emscripten_dir is None:
        log.error(
            "Cannot get Emscripten path, please use 'emcmake' launcher or specify it either by EMSCRIPTEN/EMSDK environment variable or --emscripten_dir option."
        )
        sys.exit(-1)
